...
if [[ $CHANGED_COUNT > 0 ]]; then
  STATUS_CHANGED="#[fg=#dca561,bg=#080808,bold] ${CHANGED_COUNT} "
fi

if [[ $INSERTIONS_COUNT > 0 ]]; then
  STATUS_INSERTIONS="#[fg=#44dfaf,bg=#080808,bold] ${INSERTIONS_COUNT} "
fi

if [[ $DELETIONS_COUNT > 0 ]]; then
  STATUS_DELETIONS="#[fg=#e46876,bg=#080808,bold] ${DELETIONS_COUNT} "
fi

if test "$BRANCH" != ""; then
  if test "$STATUS" = "0"; then
    echo "#[fg=#44dfaf,bg=#080808,bold]🮐  $RESET$BRANCH "
  else
    echo "#[fg=#d27e99,bg=#080808,bold]🮐  $RESET$BRANCH $RESET$STATUS_CHANGED$RESET$STATUS_INSERTIONS$RESET$STATUS_DELETIONS"
  fi
fi
